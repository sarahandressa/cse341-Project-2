...
            console.error('POST /posts Failed with response body:', response.body);
        }
...
    test('PUT /posts/:id should allow the owner to update the post (200)', async () => {
        if (!postId) throw new Error("postId is undefined, POST test failed to capture ID.");

        const updatedPost = {
            // FIX: Include required fields (club, title, content) to prevent 400 Validation Error
            club: clubId,
            title: 'First Post Test', 
            content: 'Updated content for the test post.', // This field is actually updated
        };
...
        const updatedResult = response.body.post || response.body.updatedPost || response.body;
        expect(updatedResult.content).toBe(updatedPost.content);
    });

    // Test 4: DELETE /posts/:id (Delete)
    test('DELETE /posts/:id should allow the owner to delete the post (200)', async () => {
        if (!postId) throw new Error("postId is undefined, POST test failed to capture ID.");
